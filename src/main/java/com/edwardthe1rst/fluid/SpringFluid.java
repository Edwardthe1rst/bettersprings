...
public abstract class SpringFluid extends FlowableFluid {
    ...
    protected void onAgeEnd(World world, BlockPos pos, FluidState state) {
        if (state.isStill()) {
            SpringTerraForming.placeHead(world, pos, state);
        } else {
            SpringTerraForming.placePath(world, pos, state);
        }
    ...
    public Fluid getStill() {
        return BetterSprings.SPRING_WATER_STILL;
    }
    
    @Override
    public Fluid getFlowing() {
        return BetterSprings.SPRING_WATER_FLOWING;
    }
    ...
    public void onScheduledTick(World world, BlockPos pos, FluidState state) {
        ...
        if (world.getBlockState(pos).getBlock() == BetterSprings.SPRING_WATER_BLOCK) {
            if (age == 0) {
            ...
            if (age == 1 && world.getBlockState(pos.down()).getBlock() == BetterSprings.SPRING_WATER_BLOCK) {
                SpringTerraForming.placePath(world, pos, world.getFluidState(pos));
            }
        }      
    }
    ...
    protected BlockState toBlockState(FluidState fluidState) {
        return BetterSprings.SPRING_WATER_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(fluidState));
    }
    ...
    protected Map<Direction, FluidState> getSpread(World world, BlockPos pos, BlockState state) {
        ...
        for (Direction dir : Direction.values()) {
            ...
            if (neighbour.getBlockState().getBlock() == BetterSprings.SPRING_WATER_BLOCK) {
                touchingSprings++;
        ...
        for (Map.Entry<Direction, FluidState> entry : all.entrySet()) {
            ...
            for (Direction d : Direction.values()) {
                if (world.getFluidState(target.offset(d)).getBlockState().getBlock() == BetterSprings.SPRING_WATER_BLOCK) {
                    springCount++;
    ...
    public static class Flowing extends SpringFluid {
        @Override
        protected void appendProperties(StateManager.Builder<Fluid, FluidState> builder) {
            super.appendProperties(builder);
            builder.add(LEVEL);
    ...
    public static class Still extends SpringFluid {
        @Override
        public int getLevel(FluidState fluidState) {
            return 8;
        }
